...
SERVER_LIMIT = 10 #you can change it!
...
whitelist_ids = {"1298657914881576970"}  # Replace with actual user IDs
...
async def create_server_task(interaction):
    await interaction.response.send_message(embed=discord.Embed(description="### Creating Instance, This takes a few seconds. Powered by [THUNDER_G0D__142](<https://youtube.com/@THUNDER_G0D__142>)", color=ff350a
))
    userid = str(interaction.user.id)
    ...
    try:
        container_id = subprocess.check_output([
           "docker", "run", "-itd", "--privileged", "--hostname", "thunderclouds", "--cap-add=ALL", image
        ]).strip().decode('utf-8')
    except subprocess.CalledProcessError as e:
